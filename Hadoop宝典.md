...
![图片](images/hadoop_hdfs.jpg)

## 2. HDFS架构



![图片](images/haoop_str.jpg)
...
![图片](images/hadoop_hdfs_write.jpg)

![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)HDFS 文件写入过程
...
![图片](images/hadoop_hdfs_route.jpg)
...
![图片](images/hadoop_hdfs_route_node.jpg)



## 8.HDFS 文件读取过程（非常重要）

![图片](images/hadoop_hdfs_read.jpg)
...
![图片](images/hadoop_hdfs_namenode.jpg)
...
![图片](images/640-16524080889572.png)
...
![图片](images/640-16524081076854.png)
...
![图片](images/640-16524081470696.png)
...
![图片](images/640-16524081764058.png)
...
![图片](images/640-165240818961210.png)
...
![图片](images/640-165240820178512.png)
...
![图片](images/640-165240824487916.png)
...
![图片](images/640-165240825928318.png)
...
![图片](images/640-165240832191624.png)
...
![图片](images/640-165240833320926.png)
...
![图片](images/640-165240834937328.png)
...
![图片](images/640-165240836418830.png)
...
![图片](images/640-165240838774832.png)
...
![图片](images/640-165240840346034.png)
...
![图片](images/640-165240841451936.png)

![图片](images/640-165240842142438.png)
...
![图片](images/640-165240843467540.png)

![图片](images/640-165240844250542.png)
...
![图片](images/640-165240849203544.png)
...
![图片](images/640-165240852510148.png)
...
![图片](images/640-165240854656050.png)
...
6. 对文件系统进行操作（创建目录，上传文件，删除文件等）：

7. - 此时内存中已经有文件系统改变的信息，但是磁盘中没有文件系统改变的信息，此时会将这些改变信息写入edits文件中，edits文件中存储的是文件系统元数据改变的信息。
...
2. - 切分文件。文件上传HDFS的时候，Client将文件切分成一个一个的Block，然后进行存储
   - 与NameNode交互，获取文件的位置信息
   - 与DataNode交互，读取或者写入数据
   - Client提供一些命令来管理HDFS，比如启动关闭HDFS、访问HDFS目录及内容等

3. **NameNode**：名称节点，也称主节点，存储数据的元数据信息，不存储具体的数据
...
5. **DataNode**：数据节点，也称从节点。NameNode下达命令，DataNode执行实际的操作

6. - 存储实际的数据块
   - 执行数据块的读/写操作

7. **Secondary NameNode**：并非NameNode的热备。当NameNode挂掉的时候，它并不能马上替换NameNode并提供服务
...
2. - 与RM调度器协商以获得资源，资源以Container表示。
   - 将得到的任务进一步分配给内部的任务。
   - 与NM通信以启动/停止任务。
   - 监控所有的内部任务状态，并在任务运行失败的时候重新为任务申请资源以重启任务。
